...
userList ImportUsers::dispPersonList() { return this->people; }
...
void ImportUsers::buttonStub() {
  ...
  QMessageBox::information(
      ...
      "This button doesn't do anything in this release: " + versionNum);
}
 ...
 */
// Give a third option to populate these fields if the settings file was
// corrupted Then have this file be saved in the program's directory to be used
// again
int ImportUsers::settingsFile() {  // Load settings from default settings file
  ...
  while (retval != SUCCESS) {
    ...
    QPushButton *SettingsOpen =
        openSetFile.addButton("Open Settings File...", QMessageBox::ActionRole);
    QPushButton *TerminateProg =
        openSetFile.addButton("Terminate program", QMessageBox::ActionRole);
    openSetFile.setWindowTitle("Settings file Missing...");
    openSetFile.setText(
        "Please select a settings file to use or terminate program to cancel.");
    openSetFile.exec();
    if (openSetFile.clickedButton() == SettingsOpen) {
      fileName =
          QFileDialog::getOpenFileName(this, "Settings file to Open", "",
...
QString ImportUsers::saveToFile() {
  ...
  QString fileName =
      QFileDialog::getSaveFileName(this, "Export User Files to a csv...", "",
  ...
  if (fileName.isEmpty())
    ...
    if (retval == SUCCESS) {
      ...
      QMessageBox::information(this, "Failed to Export file",
...
void ImportUsers::loadFromFile() {
  QString fileName =
      QFileDialog::getOpenFileName(this, "Open New Users to import...", "",
  ...
  if (fileName.isEmpty())
    ...
    if (people)
      QMessageBox::information(this, "Imported a file",
                               "Sucessful import of " + fileName);
    else
      QMessageBox::information(this, "Failed to Import file",
                               "Failed to import from " + fileName);
  }
}
...
void ImportUsers::importToAD() {
  ...
  if (fileName.isNull())
    QMessageBox::information(this, "Failed to Save File",
                             "Did not import to AD. Please try again.");
  else {
    QString scriptName = QFileDialog::getOpenFileName(
        this, "Select script to execute...", "",
 ...
 *      'deleteAllUsers.' It allows the user to delete all the users on the
 * heap.
 */
void ImportUsers::deleteUserList() {
  ...
  int delval;
  int ret = QMessageBox::warning(this, "Delete all users",
  ...
  switch (ret) {
    case QMessageBox::Ok:
      ...
      if (delval == SUCCESS)
        ...
        QMessageBox::information(this, "Deletion Unsuccessful",
