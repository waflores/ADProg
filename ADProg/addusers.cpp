...
void AddUsers::submitAddForm() {
  ...
  for (columnCount = 0; columnCount < 11; ++columnCount) {
    ...
    if (formStr.trimmed().isNull() || formStr.trimmed().isEmpty())
      inputArray << "*";
    else
      inputArray << formStr.trimmed();
  }
  ...
  //            QPushButton * omitField = cancelConfirm.addButton("Omit",
  //            QMessageBox::ActionRole); QPushButton * exitAdd =
  //            cancelConfirm.addButton(QMessageBox::Abort);
  //            cancelConfirm.setWindowTitle("Leave Blank or Exit?");
  //            cancelConfirm.setText("Click \"Omit\" to leave field blank or
  //            \"Abort\" to cancel adding a user."); cancelConfirm.exec();
  //
  ...
  //                QMessageBox::information(this, "User adding cancelled",
  //                "User was not added."); return; // abort adding a person
  //            }
  //        }
  //    }
  ...
  if (retval != NULL)
    QMessageBox::information(this, "User added",
                             inputArray.at(0) + " was successfully added.");
  else
    QMessageBox::information(this, "User not added",
                             inputArray.at(0) + " was not added.");
...
void AddUsers::clearAddForm(bool warn) {
  int ret;
  if (warn)
    ret = QMessageBox::warning(this, "Reset Form",
