...
/*
 * EditUsers Class: This class implements the editing layout for the user
 * interface.
 */
EditUsers::EditUsers(ImportUsers *prog, QWidget *parent)
...
void EditUsers::deletePerson() {
  int ret;
  ret = QMessageBox::warning(this, "Delete this User",
                             "Are you sure you want to delete this user?",
                             QMessageBox::Cancel, QMessageBox::Ok);
  switch (ret) {
    case QMessageBox::Ok:
      current_person = deleteOneUser(current_person);
      QMessageBox::information(this, "Deletion Successful!",
                               "User was deleted.");
      if (current_person)
        displayEdits(current_person);
      else {
        this->close();
        QMessageBox::information(this, "Deleted all users...",
...
void EditUsers::editPerson() { updateInterface(EDIT_MODE_BUTTONS); }
...
void EditUsers::cancelChange() { updateInterface(NAVIGATION_MODE_BUTTONS); }
...
void EditUsers::processChanges() {
  ...
  if (uNameEdit->isModified()) {
    char *retval = NULL;
    retval = changeUserName(uNameEdit->text().toStdString().c_str(),
                            current_person->user);
    changeProfilePath(uNameEdit->text().toStdString().c_str(),
                      current_person->user);
    changeHomeDirectory(uNameEdit->text().toStdString().c_str(),
                        current_person->user);
    if (retval)
      QMessageBox::information(this, "User Name changed!",
                               "Username has been changed.");
    else
      QMessageBox::information(this, "User Name not changed!",
                               "Username has not been changed.");
  }
  if (FNameEdit->isModified()) {
    char *retval = NULL;
    retval = changeFirstName(FNameEdit->text().toStdString().c_str(),
                             current_person->user);
    if (retval)
      QMessageBox::information(this, "First Name changed!",
                               "First name has been changed.");
    else
      QMessageBox::information(this, "First Name not changed!",
                               "First name has not been changed.");
  }
  if (MNameEdit->isModified()) {
    char *retval = NULL;
    retval = changeMiddleName(MNameEdit->text().toStdString().c_str(),
                              current_person->user);
    if (retval)
      QMessageBox::information(this, "Middle Name changed!",
                               "Middle name has been changed.");
    else
      QMessageBox::information(this, "Middle Name not changed!",
                               "Middle name has not been changed.");
  }
  if (LNameEdit->isModified()) {
    char *retval = NULL;
    retval = changeLastName(LNameEdit->text().toStdString().c_str(),
                            current_person->user);
    if (retval)
      QMessageBox::information(this, "Last Name changed!",
                               "Last name has been changed.");
    else
      QMessageBox::information(this, "Last Name not changed!",
                               "Last name has not been changed.");
  }
  if (deptEdit->isModified()) {
    char *retval = NULL;
    retval = changeDepartment(deptEdit->text().toStdString().c_str(),
                              current_person->user);
    if (retval)
      QMessageBox::information(this, "Department changed!",
                               "Department has been changed.");
    else
      QMessageBox::information(this, "Department not changed!",
                               "Department has not been changed.");
  }
  if (InitEdit->isModified()) {
    char *retval = NULL;
    retval = changeInitials(InitEdit->text().toStdString().c_str(),
                            current_person->user);
    if (retval)
      QMessageBox::information(this, "Initials changed!",
                               "Initials have been changed.");
    else
      QMessageBox::information(this, "Initials not changed!",
                               "Initials have not been changed.");
  }
  if (titleEdit->isModified()) {
    char *retval = NULL;
    retval = changeTitle(titleEdit->text().toStdString().c_str(),
                         current_person->user);
    if (retval)
      QMessageBox::information(this, "Title changed!",
                               "Title has been changed.");
    else
      QMessageBox::information(this, "Title not changed!",
                               "Title has not been changed.");
  }
  if (pNameEdit->isModified()) {
    char *retval = NULL;
    retval = changePreferredName(pNameEdit->text().toStdString().c_str(),
                                 current_person->user);
    if (retval)
      QMessageBox::information(this, "Preferred Name changed!",
                               "Preferred Name has been changed.");
    else
      QMessageBox::information(this, "Preferred Name not changed!",
                               "Preferred Name has not been changed.");
  }
  if (officeNameView->isModified()) {
    // special case, needs to be carefully debugged
    Location retval = NULL;
    retval = changeOfficeName(officeNameView->text().toStdString().c_str(),
                              current_person->user);
    if (retval)
      QMessageBox::information(this, "Office Name changed!",
                               "Office Name has been changed.");
    else
      QMessageBox::information(this, "Office Name not changed!",
                               "Office Name has not been changed.");
  }
  ...
  if (ModCheck->isEnabled()) {
    if (ModCheck->isChecked())
      changeMod("true", current_person->user);
    else
      changeMod("false", current_person->user);
    //(strcmp(uNameEdit->text().toStdString().c_str(), userName))
  }
  if (EmailEdit->isModified()) {
    char *retval = NULL;
    retval = changeEmail(EmailEdit->text().toStdString().c_str(),
                         current_person->user);
    if (retval)
      QMessageBox::information(this, "Email address changed!",
                               "Email has been changed.");
    else
      QMessageBox::information(this, "Email address not changed!",
                               "Email has not been changed.");
  }
  if (passwordView->isModified()) {
    char *retval = NULL;
    retval = changePassword(passwordView->text().toStdString().c_str(),
                            current_person->user);
    if (retval)
      QMessageBox::information(this, "Password changed!",
                               "Password has been changed.");
    else
      QMessageBox::information(this, "Password not changed!",
