...
/* helperfuncs.c: This file implements the functions that use standard library
 * types and functionalities. This helps with being able to modularize the code
 * for other functions in the future that require these functionalities.
 */
...
char **Split(const char *input, const char delim) {
  ...
  while (*inStr) {
    ...
    /* check for the end of the buffer */
      ...
      if (inStr - prevStr < 1)
        output[i] = emptyStr(); /* Empty string case */

      else
        output[i] =
            fillStr(prevStr, inStr - 1, delim); /* Unempty string case */
      *inStr = 0; /* Break out of the tokenization loop */
    }
    /* for all other inputs */
    else
      inStr++; /* Keep moving along the input string */
 ...
 *   end pointer, ePtr. The string is trimmed of leading whitespaces and
 * trailing whitespaces before returned to the caller. Prints error message and
 * returns NULL if string cannot be generated.
 */
/* FIX FOR 1 CHARACTER ALGORITHMS */
char *fillStr(char *sPtr, char *ePtr, const char delim) {
  char *s = NULL;       /* string to be returned after trimming and trailing
  ...
  if (sPtr && ePtr) {
    ...
    printf(
        "fillStr_error: One of the arguments are NULL, recheck function "
        "call.\r\n");
    fflush(stdout);
  ...
  if ((ePtr - sPtr) < 0)
    return emptyStr(); /* Gracefully handles fields that are only spaces */
  /* Single character token fix */
  else if (((ePtr - sPtr) == 0) && (*ePtr == delim))
    return emptyStr();
...
char *readLine(FILE *stream, unsigned int limit) {
  ...
  while (isspace(*buffer)) {
    if (buffer < endBuffer)
      ++buffer; /* trim the leading white space*/
    else
      break;
  }
  while (isspace(*endBuffer)) {
    if (endBuffer > buffer)
      --endBuffer; /* trim the ending white space*/
    else
      break;
  }
...
/* getOutFile: retrieves the output file from the user. Returns the file
 * pointer.
 */
FILE *getOutFile() {
